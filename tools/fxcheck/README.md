...
## ボス戦で 字を 書く ときの 反応（v14.16）

| ファイル | すること | 使い方 |
|---|---|---|
| `writeprobe.js` | ほんものの ゆびを CDP で 60回/秒 動かし、**canvas が 何点 ひろえたか**（＝線が どれだけ 飛ぶか）と スレッドの いそがしさを 出す | `CPU=4 DUR=3000 MINFACE=250 node writeprobe.js last 3 "まえ=@C:/mqtNNN" "あと=@C:/mqtMMM"`。変えかたは `名前=<@フォルダ>\|css=…`、`M:<モード>;` を 先頭に つけると その 行だけ モードを 変えられる（ふつうの たたかいと ボスを 交互に） |
| `animwho.js` | いま 動いて いる アニメを 中身べつに 数える（どの 部品が 何個 動いて いるか） | `node animwho.js last` |
| `glowcount.js` | モンスター 1体ずつ「光る ところ（`.bx__glow`）」の 数（310体） | `node glowcount.js` |
| `shotboss.js` | ラスボスの 3D と「書いて いる あいだの 2D」を 撮って くらべる | `node shotboss.js <版の フォルダ> last <出す フォルダ>` |

**測り方の わな（2回 まちがえた。`docs/v14.16ボス戦の書きごこちメモ.md` §4）**
- **v12.7 の「ふつう／本気」パネルが メモを ふさぐ**（`elementFromPoint` が BODY＝指が どこにも 当たって いない のに 測れて しまう）→ さわる 前に `.bosspick__btn--norm` を click。
- **`#last` は さいしょの 約18秒が 前座の ザコ**（24〜49面）。ラスボスは 360面 → **`.foes .v3 .f` が 250まいに なるまで 待つ**（`MINFACE`）。
- **1点ずつ 返事を 待つ 測り方は 本物の 指と ちがう**（ページの おそさが そのまま 待ち時間に 化ける）→ 返事を 待たずに 60回/秒。
- この PC は 空いて いる ときは **問題が 再現しない**（同じ 条件で 1.4〜8.9秒）→ かならず 交互に・中央の 値。

## わざの あいだの 重さ（v14.15）

| ファイル | すること | 使い方 |
|---|---|---|
| `spwarm.js` | **カットインの 絵が できあがって から** わざを 出して はかる（＝本物に ちかい じょうたい）。コマ・止まった いちばん 長い あいだ・Paint／Layout・面の 数 | `node spwarm.js last bolt,blaze 2`（mode＝battle／boss／last）。5つめに `"|css=…"` を つけると 部品を 外して くらべられる。`COLD=1` で わざと 絵が できる 前に 出す |

- **`#perffx`・`#fx` は わざを 出すのが 早すぎる**（たたかいの 1.5秒後・0.4秒後）＝カットインの 絵が まだ できて いない ので、
  重い ほうの 3D（面 167まい）で はかって しまう。**わざの 重さは spwarm.js で**。
- 面が だいたい **270まで なら なめらか**（ふつうの たたかい 211・エリアボス 267）。ラスボス戦は 527＝わざを 出すと 4分の1に なる。
