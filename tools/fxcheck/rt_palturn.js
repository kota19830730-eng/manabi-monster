// 相棒の ターン（v14.37）を 本当の 時計で うごかして とちゅうを 撮る（先に node build_palturn_demo.js）
// node rt_palturn.js <arm|turn|k-<kind>> <ms,ms,...> <出す名前> [W] [H]
const { spawn } = require('child_process');
const http = require('http');
const fs = require('fs');
const os = require('os');
const path = require('path');
const [btn, times, name] = process.argv.slice(2);
...
(async function () {
  ...
  await send('Page.navigate', { url: 'file:///' + __dirname.replace(/\\/g, '/') + '/palturn.html' });
  await sleep(4500);
  if (btn !== 'arm') { await send('Runtime.evaluate', { expression: "document.querySelector('.mihon__b--arm').click(); 1" }); await sleep(900); }   // 先に かまえ
  const t0 = Date.now();
  await send('Runtime.evaluate', { expression: "document.querySelector('.mihon__b--" + btn + "').click(); 1" });
  ...
  const e = await send('Runtime.evaluate', { expression: "JSON.stringify({err: (window.MQ && MQ.guard && MQ.guard.list) ? MQ.guard.list() : null, fxc: MQ.ui.fxc.state(), turn: document.querySelector('#screen-battle .arena').classList.contains('is-palturn'), back: document.querySelector('#screen-battle .hero').classList.contains('is-back')})", returnByValue: true });
