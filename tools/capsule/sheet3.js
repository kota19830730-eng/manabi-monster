/* カプセル専用モンスター 18系統を 1まいの HTML に ならべて 見る。
   node tools/capsule/sheet.js  →  tools/capsule/sheet3.html
...
const html = '<!doctype html><html><head><meta charset="utf-8"><title>カプセル専用モンスター</title>'
  ...
  + '</style></head><body>'
  + '<h1>カプセル専用モンスター 第2弾（1段階め＋シークレット）</h1>'
  + group('ふつう ・ ようかい隊', defs.YOKAI, '#9fb2dd')
  + group('レア ・ メカ生きもの', defs.MECHA, '#ffd447')
  + group('げきレア ・ でんせつの どうぶつ', defs.LEGEND, '#b48cff') + group('シークレット（？？？）', defs.SECRET, '#72f0ff')
  + '</body></html>';

const out = path.join(__dirname, 'sheet3.html');
fs.writeFileSync(out, html);
console.log('wrote ' + out + '（' + (defs.ALL2.length + defs.SECRET.length) + '体・気に なる ところ ' + allWarns + '）');
