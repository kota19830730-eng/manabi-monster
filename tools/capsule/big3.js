/* カプセル専用モンスター 18系統を 1まいの HTML に ならべて 見る。
   node tools/capsule/sheet.js  →  tools/capsule/big3.html
...
function card(m) {
  const big = box(m.shape, m.colors, 288);
...
const html = '<!doctype html><html><head><meta charset="utf-8"><title>カプセル専用モンスター</title>'
  ...
  + group('大きく', [defs.YOKAI[3], defs.MECHA[1], defs.LEGEND[0], defs.YOKAI[0]], '#ffd447')
  + '</body></html>';

const out = path.join(__dirname, 'big3.html');
