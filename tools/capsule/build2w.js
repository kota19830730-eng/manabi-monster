/* カプセル専用モンスターの 進化形 36体（2・3段階め）を 組み立てて 一覧に する。
     node tools/capsule/build.js   →  tools/capsule/sheet4.html と evo2-arts.json
...
const defs = require('./defs2.js');
defs.ALL = defs.ALL2;
const evo = require('./evo2.js');
...
function lift(colors, k) {
  if (!k) return Object.assign({}, colors);   // 第2弾：cx だけの 段（lift なし）で もとの 色を 書きかえない（apply.js と 同じ）
...
evo.forEach(function (line) {
  ...
  line.steps.forEach(function (d) {
    ...
    /* ④ 意図しない まばたき（第2弾：カラカサンの 一つ目は わざと まばたく ので、1段階めより ふえた ときだけ 直す） */
    const blink = Object.keys(MQ.blocks.pickEyesTest(art)).length;
    const blinkBase = Object.keys(MQ.blocks.pickEyesTest(base.shape)).length;
    if (blink > blinkBase) problems.push(d.id + ' 意図しない まばたき ' + blink + 'こ（1段階めは ' + blinkBase + '）');
...
fs.writeFileSync(path.join(__dirname, 'sheet4.html'), html);
fs.writeFileSync(path.join(__dirname, 'evo2-arts.json'), JSON.stringify(arts, null, 1));
