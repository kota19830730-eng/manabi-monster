/* ---------------------------------------------------------
   ...
   VOX2.chest({ unit, open, shadow }) → .v3.v3--chest（open なら is-open＝ふたが ひらいた まま）
...
(function () {
  ...
  function chest(opts) {
    ...
    const v = VOX2.fromGroups(groups, { unit: opts.unit || 2, shadow: opts.shadow });
    ...
    v.querySelectorAll('.p--lid > .b').forEach(function (b) {
      ...
      under.style.backgroundImage = 'url(' + VOX2.texture(cols, Math.max(2, DEPTH), 'top', 'y') + ')';
    });
  ...
  window.VOX2.chest = chest;
  window.VOX2.chestShapes = { LID: LID, BASE: BASE, COINS: COINS, COLORS: COLORS, DEPTH: DEPTH };
})();
