/* タイトル画面の 勇者の ドット絵を 作る スクリプト（node tools/gen-title-art.js）
   ...
   v10.0（2026-09-07）から この 絵は **HD（1マスを 2×2 で 描く）**で 出します。
   pixel.js の `region()` は **大文字と 小文字を 1つの かたまり**と 見なすので、
   同じ ものの「明るい面／くらい面」は **A と a のように ペアの 文字**に します。
   ペアに しないと、その さかいめに もう 一本 ふちの 明暗が 入って
   「二重線」に 見えます（よろいの 明るい すじ L → **a** に したのは この ため）。
...
function knight() {
  ...
  /* ---- 右うで（けんを かかげる）---- */
  rect(g, 26, 15, 5, 8, 'A');
  rect(g, 26, 15, 5, 1, 'a');            // よろいの 明るい すじ（**大文字と 小文字で ペア**。下の メモ）
  rect(g, 30, 15, 1, 8, 'D');

  /* ---- かみの毛 と 顔（v9.8 で 直した）----
     ユーザー「勇者ダサいから修正お願い」。実測すると **かみ（K）が 108マスで
     いちばん 多く**、頭の 上 6行が こげ茶の べた塗り＝「茶色の かたまり」に
     見えて いた。顔は よこ 10マスしか なく、左右の かみ 2列に はさまれて 小さい。
     マイクラの 頭は「顔が ほとんど・かみは 上の うすい かぶり」なので、
       かみ 6行 → **4行**／顔 よこ10・たて8 → **よこ12・たて10**／よこの かみ 2列 → **1列**
     に して、かみに 明るい すじ（k）を 入れ べた塗りを やめた（K 108 → 60マス）。 */
  rect(g, 11, 4, 14, 4, 'K');            // かみ（上）
  rect(g, 11, 4, 14, 1, 'k');            // 上の 明るい 行
  px(g, 13, 5, 'k'); px(g, 18, 5, 'k'); px(g, 22, 5, 'k');   // かみの すじ
  px(g, 15, 6, 'k'); px(g, 20, 6, 'k');
  rect(g, 12, 8, 12, 10, 's');           // 顔
  rect(g, 22, 8, 2, 10, 'e');            // 右の かげ
  rect(g, 11, 8, 1, 8, 'K');             // よこの かみ
  rect(g, 24, 8, 1, 8, 'K');
  rect(g, 12, 8, 2, 1, 'K');             // まえがみの ぎざぎざ
  rect(g, 16, 8, 2, 1, 'K');
  rect(g, 20, 8, 3, 1, 'K');
  rect(g, 14, 11, 3, 3, 'B');            // 青い 目（左上に 白い 光）
  px(g, 14, 11, 'W');
  rect(g, 19, 11, 3, 3, 'B');
  px(g, 19, 11, 'W');
