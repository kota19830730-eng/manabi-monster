...
MQ.ui.dex = (function () {
  ...
  function render(which) {
    // おうちの人ページは べつの 画面（v7.3）。せっていの 部品が render('parent') を 呼んだら そちらを 書き直す
    if (which === 'parent' && MQ.ui.parent) {
      if (MQ.ui.parent.isOpen()) MQ.ui.parent.refresh(); else MQ.ui.parent.open('settings');
      return;
    }
  ...
  /* おうちの人ページは js/ui/parent.js（v7.3・べつの 画面）。ここには 設定の 部品だけ のこす */
  ...
  function feverSection(player) {
    ...
    return h('div', { class: 'pfever' }, [
      ...
      h('h3', { class: 'label', text: 'てきの こうげき・ボスの わざ' }),
      h('p', { class: 'note', text: 'ザコは 3問ごと・ボスは 2問ごとに「こうげき」して きます。その 問題に 1回めで 正解すると カウンター（けいけんち 1.5倍・ボスには 2ダメージ）。ボスは 3問めから「たての かまえ」「ぶんしん」「なかまを よぶ」の わざも 使います（どれも 正解すると ボーナス）。まちがえても 演出だけで、うしなう ものは ありません。こわがる 子には「なし」に できます（中ボスと 弱点は そのまま）。' }),
  ...
  /* =======================================================
     せってい（音・きろくの ほぞん）
     ======================================================= */
  function recordsSection(player) {
