...
MQ.hero = (function () {
  ...
  /* =======================================================
     ...
     v5.4：グレード 6（かわ／てつ／りゅう／でんせつ／ほし／やみ）・そうびに 戦いの 効果（下の GEAR_POWER）
     v9.0：カプセル限定の 2グレード（カプセル／オーロラ）
     v13.19：**絵を 作り直した**（ユーザー「装備品を もっと 見た目を カッコ良く」2026-09-14）。
       前は「てつ」の 形に 四角を 2〜6個 足しただけで、どの グレードも 色ちがいに 見えた。
       いまは js/content/gearart.js（MQ.gearArt）が グレードごとに りんかくから ちがう 形を 作る。
       ・たては 左うでの よこ（体を ふさがない）・けんは 太く 長く・かぶとは ひたい当てと ほほ当て
       ・大きな かた当ては 左かただけ（右は けんの 場所）・かぶとは x11〜35 の 中だけ（3D の 頭の はんい）
       ・5点 そろえると グレードごとの オーラ（js/ui/gearaura.js）
       見くらべは tools/gearlab/lab.html。
     ======================================================= */
  ...
  const GEAR_DEF = {
    ...
    densetsu: {
      weapon: { name: 'ひかりのけん' },
      shield: { name: 'ゆうしゃの たて' },
      helm:   { name: 'ひかりの かぶと' },
      armor:  { name: 'ひかりの よろい' },
      cape:   { name: 'でんせつの マント' }
    },
    hoshi: {
      weapon: { name: 'ほしくずの けん' },
      shield: { name: 'ほしの たて' },
      helm:   { name: 'ほしの かぶと' },
      armor:  { name: 'ほしの よろい' },
      cape:   { name: 'ぎんがの マント' }
    ...
    capsule: {
      weapon: { name: 'カプセルの けん' },
      shield: { name: 'カプセルの たて' },
      helm:   { name: 'カプセルの かぶと' },
      armor:  { name: 'カプセルの よろい' },
      cape:   { name: 'カプセルの マント' }
    },
    // げきレアの わく。むらさき＝アバターの SR バッジ・カプセルの げきレアと 同じ 色
    aurora: {
      weapon: { name: 'オーロラの けん' },
      shield: { name: 'オーロラの たて' },
      helm:   { name: 'オーロラの かぶと' },
  ...
  grades.forEach(function (g) {
    ORDER.forEach(function (slot) {
      ...
      gear.push({
        ...
        gradeName: g.name,
        name: def.name,
        rows: MQ.gearArt.rows(slot, g.id),
        palette: MQ.gearArt.palette(g.id, slot),
        mat: MQ.gearArt.mat(g.id, slot),
        capsule: !!g.capsule,
  ...
  /* かぶとの 下（v13.19）：ひたい当て（行10）より 上の かみ・かざりを けす。
     けさないと、とがった かみが かぶとの 上から 出て「かぶとが かみに 見える」（前の そうびの 問題の 1つ） */
  const HELM_TOP = 10;
  function underHelm(layer) {
    const out = Object.assign({}, layer);
    out.rows = layer.rows.map(function (r, y) { return y <= HELM_TOP ? r.replace(/[^.]/g, '.') : r; });
    if (layer.rows2) out.rows2 = layer.rows2.map(function (r, y) { return y <= HELM_TOP * 2 + 1 ? r.replace(/[^.]/g, '.') : r; });
    return out;
  }
  ...
  function layersFor(player, opts) {
    ...
    if (cape) list.push({ rows: cape.rows, palette: cape.palette, mat: cape.mat, gear: 'cape' });
    ...
    const helmOn = !!(eq.helm && gearById[eq.helm]);
    list.push(helmOn ? underHelm({ rows: hair.rows, rows2: hair.rows2, palette: P.hair, mat: MAT.hair })
                     : { rows: hair.rows, rows2: hair.rows2, palette: P.hair, mat: MAT.hair });
    const acc = { rows: F.pick(F.accStyles, look.acc).rows, palette: P.acc, mat: MAT.acc };
    list.push(helmOn ? underHelm(acc) : acc);
    ...
    ['armor', 'helm', 'shield', 'weapon'].forEach(function (slot) {
      const g = eq[slot] && gearById[eq[slot]];
      if (g) list.push({ rows: g.rows, palette: g.palette, mat: g.mat, gear: slot });
    });
    return list;
  }

  /* 3D の 部品分け（v13.19）。48×48 の マスごとに、いちばん 手前に 見えて いる マントの マス（x33 より 右だけ）に 'cape'。
     vox.js の fromHero は 列で 部品を 分ける（x33〜 右うで・x36〜 けん）ので、右に 広がる マントが
     うで・けんと いっしょに 回って しまう。そこだけ 体の 部品へ うつす。
     ぜんぶの マスに しるしを つけると 箱が こまかく 切れて 面が 2ばいに なった（1コマ ＋10ms）ので、右の マントだけ。 */
  const LABEL_X = 33;
  function labels(player) {
    const out = new Array(48 * 48).fill(null);
    let any = false;
    layersFor(player).forEach(function (l) {
      const tag = l.gear === 'cape' ? 'cape' : null;
      l.rows.forEach(function (r, y) {
        for (let x = 0; x < 48; x++) if (r[x] && r[x] !== '.') { out[y * 48 + x] = tag && x >= LABEL_X ? tag : null; if (tag && x >= LABEL_X) any = true; }
      });
    });
    return any ? out : null;
  }
  // いま つけて いる 一式の グレード id（オーラ用・v13.19）。そろって いなければ null
  function fullSetGrade(player) { const g = equippedSetOf(player); return g ? g.id : null; }
  ...
  function sprite(player, opts) {
    ...
    return MQ.pixel.url(keyFor(player, opts) + ':hd4', layersFor(player, opts), HD_OPTS());
  }
  ...
  function gearSprite(id) {
    ...
    return MQ.pixel.url('gear:hd3:' + id, [
      ...
      { rows: g.rows, palette: g.palette, mat: g.mat }
    ], HD_OPTS());
  }
